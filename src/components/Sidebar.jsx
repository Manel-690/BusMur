...
export default function Sidebar() {
  ...
  const menu = ROLE_MENUS[user?.role] || ROLE_MENUS.passenger;

  return (
    <>
      <button onClick={() => setOpen(!open)} className="lg:hidden fixed top-4 left-4 z-50 p-2.5 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 shadow-lg">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-5 h-5">
      ...
      <aside className={`fixed lg:sticky top-0 left-0 z-40 h-screen w-64 flex flex-col bg-white dark:bg-[#0a1120] border-r border-gray-100 dark:border-gray-800 transition-transform lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="p-3 m-3 rounded-2xl bg-gray-50 dark:bg-gray-800/40 border border-gray-100 dark:border-gray-700/50">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center text-white text-sm font-bold shrink-0" style={{ background: ROLE_COLORS[user?.role] || "#1d4ed8" }}>{user?.name?.[0] || "?"}</div>
            <div className="min-w-0 flex-1"><p className="text-xs font-bold truncate text-gray-800 dark:text-white">{user?.name}</p><span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full" style={{ background: ROLE_COLORS[user?.role] + "22", color: ROLE_COLORS[user?.role] }}>{ROLE_LABELS[user?.role]}</span></div>
          </div>
          <div className="flex gap-2">
            <button onClick={toggleTheme} className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-[11px] font-semibold text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700">{theme === "light" ? MoonIcon : SunIcon}{theme === "light" ? "Escuro" : "Claro"}</button>
            <button onClick={() => { logout(); navigate("/login"); }} className="flex items-center justify-center px-3 py-1.5 rounded-lg text-[11px] font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20">{LogoutIcon}</button>
          </div>
        </div>
      </aside>
    </>
