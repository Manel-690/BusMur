  ...
  return (
    <div>
      ...
      <div className="flex items-start gap-4 mb-6">
        <div
          className="w-16 h-16 rounded-2xl flex items-center justify-center text-white text-lg font-black shadow-lg mono shrink-0"
          ...
          <div className="flex items-center gap-3 mt-2">
            {linha.funcionaFimDeSemana && (
              <span className="text-xs text-purple-600 font-semibold flex items-center gap-1">
                {Ic.calendar} Funciona fim de semana
              </span>
            )}
            {linha.trocaMotorista && (
              <span className="text-xs text-gray-500 flex items-center gap-1">
                {Ic.refresh} Troca de motorista: {linha.trocaMotorista}
              </span>
            )}
          </div>
        </div>
      ...
      {linha.status !== "inativa" && proximosHorarios.length > 0 && (
        <div
          className="card p-5 mb-5 border-l-4 rounded-r-xl bg-white dark:bg-gray-900 shadow-sm"
        ...
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {proximosHorarios.map((h, i) => (
              <div
                key={h.hora}
                className={`text-center p-3 rounded-xl ${i === 0 ? "text-white shadow-md" : "bg-gray-50 dark:bg-gray-800/50"}`}
                style={i === 0 ? { background: linha.cor } : {}}
              >
                <p
                  className={`text-xl font-black mono ${i === 0 ? "text-white" : "text-gray-800 dark:text-white"}`}
                >
                  {h.diff}m
                </p>
                <p
                  className={`text-xs font-semibold mt-0.5 ${i === 0 ? "text-white/80" : "text-gray-500"}`}
                >
                  {h.hora}
                </p>
                {i === 0 && (
                  <p className="text-[10px] text-white/70 mt-0.5 uppercase tracking-wide">
                    próximo
                  </p>
                )}
              </div>
        ...
        <div className="card p-5 bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => setDiaUtil(true)}
                className={`text-xs font-bold px-3 py-1 rounded-lg transition-colors ${diaUtil ? "bg-blue-500 text-white" : "bg-gray-100 dark:bg-gray-800 text-gray-500 hover:bg-gray-200 dark:hover:bg-gray-700"}`}
              >
                Dia útil
              </button>
              {linha.funcionaFimDeSemana && (
                <button
                  onClick={() => setDiaUtil(false)}
                  className={`text-xs font-bold px-3 py-1 rounded-lg transition-colors ${!diaUtil ? "bg-purple-500 text-white" : "bg-gray-100 dark:bg-gray-800 text-gray-500 hover:bg-gray-200 dark:hover:bg-gray-700"}`}
                >
                  Fim de semana
                </button>
              )}
          ...
          {!diaUtil &&
          (!linha.horariosFimDeSemana ||
            linha.horariosFimDeSemana.length === 0) ? (
            <p className="text-sm text-gray-400 py-4 text-center">
              Esta linha não opera aos fins de semana.
            </p>
          ) : (
            <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
              {horariosAtivos.map((h) => {
                ...
                return (
                  <div
                    key={h}
                    className={`text-center py-2 px-3 rounded-xl mono text-sm font-bold ${passou ? "bg-gray-100 dark:bg-gray-800 text-gray-400" : "bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400"}`}
                  >
                    {h}
                  </div>
                );
            ...
            {linha.paradas.map((parada, i) => (
              <div key={parada} className="flex items-start gap-4">
                <div className="flex flex-col items-center shrink-0">
                  <div
                    className={`w-4 h-4 rounded-full border-2 border-white dark:border-gray-900 shadow ${i === 0 || i === linha.paradas.length - 1 ? "w-5 h-5" : ""}`}
                    style={{
                      background:
                        i === 0
                          ? "#059669"
                          : i === linha.paradas.length - 1
                            ? "#dc2626"
                            : linha.cor,
                    }}
                  />
                  {i < linha.paradas.length - 1 && (
                    <div
                      ...
                      style={{ background: linha.cor + "50" }}
                    />
                  )}
                </div>
                <div className="pb-8 flex-1">
                  <p
                    className={`text-sm font-semibold ${i === 0 ? "text-emerald-600" : i === linha.paradas.length - 1 ? "text-red-600" : "text-gray-700 dark:text-gray-300"}`}
                  >
                    {parada}
                  </p>
      ...
      {tab === "mapa" && (
        <div
          className="card overflow-hidden rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm"
          style={{ height: "420px" }}
        >
          <MapView rotaSelecionada={dadosDaRotaParaOMapa} />
        </div>
      )}
    </div>
  );
